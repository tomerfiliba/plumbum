...
* :func:`gui_open(path) <plumbum.path.utils.gui_open>` - Opens a file in the default editor on Windows, Mac, or Linux. Uses ``os.startfile`` if available (Windows), ``xdg_open`` (GNU), or ``open`` (Mac).
